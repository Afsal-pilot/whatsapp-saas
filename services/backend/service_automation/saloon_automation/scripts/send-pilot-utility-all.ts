/**
 * Send the UTILITY template `pilot_appointment_reminder` to every eligible
 * recipient — same shape as send-pilot-campaign.ts but for the utility template.
 *
 * Flow:
 ...
 *
 * Usage:
 *   npx tsx scripts/send-pilot-utility-all.ts                          # all eligible from DB
 *   npx tsx scripts/send-pilot-utility-all.ts --dry-run                # preview, send nothing
 *   npx tsx scripts/send-pilot-utility-all.ts 919840675929,918148741342
 *
...
const TEMPLATE_NAME = "pilot_appointment_reminder";
const TEMPLATE_LANG = "en_US";
const BUSINESS_NAME = "Glow Hair Studio";
const APPOINTMENT_TIME = "30 Jun 2026, 3:00 PM";
...
// Uses the shared @whatsapp-saas/wapi-meta package — no inline Meta API code.
import { sendTemplate, type TemplateMessage } from "@whatsapp-saas/wapi-meta";

type SendOk = { wa_message_id: string };
type SendErr = { code: string; message: string };
type SendResult =
  | { kind: "ok"; result: SendOk }
  | { kind: "err"; error: SendErr };

function templateFor(r: Recipient): TemplateMessage {
  return {
    name: TEMPLATE_NAME,
...
function templatePayload(r: Recipient): Record<string, unknown> {
  // Preserved for --dry-run preview output only. Actual sends go via sendTemplate().
  return {
...
async function sendOne(r: Recipient): Promise<SendResult> {
  try {
    const result = await sendTemplate(r.wa_id, templateFor(r));
    return { kind: "ok", result };
  } catch (err) {
    // wapi-meta throws on non-2xx with the Meta body in the message.
    // Extract code if present (format: "WhatsApp send_template failed (NNN): {json}").
    const message = (err as Error).message;
    const match = message.match(/"code":\s*(\d+)/);
    return {
      kind: "err",
      error: { code: match?.[1] ?? "-1", message },
    };
  }
}
...
async function main(): Promise<void> {
  ...
  console.log(`Pilot utility — template "${TEMPLATE_NAME}" (lang=${TEMPLATE_LANG})\n`);
  ...
  for (const r of recipients) {
    ...
    if (result.kind === "ok") {
      ...
      err++;
      console.log(
        `  ✗ ${r.wa_id}  ${r.name}  code=${result.error.code}: ${result.error.message}`,
      );
    }
  }
  ...
  if (!args.dryRun) {
    ...
    if (err > 0) {
      console.log(
        `Hints: code 131030 = recipient not in test-number allowlist (add them in Meta App Dashboard); code 131026 = number not on WhatsApp; code 132001 = template not approved.`,
      );
    }
  }
}
